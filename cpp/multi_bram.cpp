...
void *task0(void *arg)
{
    ...
    if ((fd = open("/dev/mem", O_RDWR | O_SYNC)) != -1)
    {
        ...
        for (int i = 0; i < brams; i++)
        {
            ...
            cout << hex << addr;
            cout << " ready\n";*/
            data[i] = (unsigned long int *)malloc(half_bram_size);
        }

        // set first data source
        for (unsigned long int i = 0; i < half_bram_size / 4; i++)
        {
            data[0][i] = i;
        }
...
void *task1(void *arg)
{
    ...
    if ((fd = open("/dev/mem", O_RDWR | O_SYNC)) != -1)
    {
        ...
        // set first data source
        for (unsigned long int i = half_bram_size / 4; i < bram_size / 4; i++)
        {
            data[0][i] = i;
        }
        ...
        {
            unsigned char *dest = (unsigned char *)axi_bram_ctrl[i];
            memcpy(dest + half_bram_size, data[i], half_bram_size);
        }
